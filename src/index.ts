...
import type { IntlCache, IntlShape, MessageDescriptor } from '@formatjs/intl';
...
export type MessageIds = FormatjsIntl.Message extends { ids: infer T } ? (T extends string ? T : string) : string;
...
export const pseudoIntl = (
	...
	intl.formatMessage = (
		...
		const message = new IntlMessageFormat(pseudoFunc(descriptor.defaultMessage), locale, undefined, opts).format(values);
